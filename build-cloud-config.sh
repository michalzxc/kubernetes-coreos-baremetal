...
- name: openstackhosts.service
  ...
  content: |
    ...
    [Service]
    ExecStart=/usr/bin/docker run --rm %NETENV% --name openstackhosts -p 53:53/udp --dns=8.8.8.8 --cap-add=SYS_ADMIN --cap-add DAC_READ_SEARCH --tmpfs /run --tmpfs /run/lock -v /sys/fs/cgroup:/sys/fs/cgroup:ro michalzxc/openstackhosts
